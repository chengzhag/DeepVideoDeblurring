...
class Deblur(object):
    def __init__(self, createFcn, maxIntensity = 255):
        ...
        self.optimizer = tf.train.AdamOptimizer(learning_rate=self.learningRatePh)
    ...
    def train(
        ...
        if lr is not None:
            self.learningRateT.load(lr, self.sess)
            print('Learning rate reseted to %f' % self.sess.run(self.learningRateT))
        ...
        if ckpDir is not None:
            ...
            tf.summary.scalar('learning rate', self.learningRateT)
        ...
        tic = time.time()
        it = tf.train.global_step(self.sess, self.global_step)
        avgSpeed = None
        while 1:
            if it >= itMax:
                break
            if it >= decayFrom and (it - decayFrom) % decayEvery == 0:
                self.sess.run(self.learningRateT.assign(self.learningRateT * decayRate))
            if self.learningRateT.eval(self.sess) < lrMin:
                self.learningRateT.load(lrMin, self.sess)
            ...
            batchLoss, _ = self.sess.run(
                ...
                feed_dict={
                    self.learningRatePh: self.learningRateT.eval(self.sess),
            ...
            print('it: %d, loss: %f, lr: %f, spd: %.2f it/s, avgSpd: %.2f it/s, left: %d h %.1f min' %
                  (it, batchLoss, self.learningRateT.eval(self.sess), speed, avgSpeed, timeLeftMin / 60, timeLeftMin % 60))
